...
// typename = void 占位 common template
namespace detail {
...
// std::void_t<typename ResultType::type>> 可以判断ResultType::type不是void
// 但不能判断RetType void 类型无法作为传参传给...可变参数

// 解决 调用返回值类型和声明返回值类型不一致问题 进行隐式转换
// declare RetType is void do nothing
template <typename ResultType, typename RetType>
...
// declare RetType is not void should convert
template <typename ResultType, typename RetType>
...
class move_only_function<RetType(ArgTypes...)>
    ...
    // use this to convert NotMoveOnlyCallable name is not proper
    ...
    // &add function pointer will instance this
    template <typename T>
    struct IsCStyleFunction : public std::false_type {};

    // add function  will instance this
...
public:
    ...
    // std::negation 模板取反逻辑
    // Functor is not move_only_function
    // Functor is not C function like "add" not "&add"
    // member function pointer same as pre one rule
    // can convert ret
    template <
        typename Functor,
        typename =
...
// 对于可调用Functor &Functor::operator()获取成员函数指针
template <
    typename Functor,

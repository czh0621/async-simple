...
public:
    ...
    struct FinalAwaiter {
        ...
        auto await_suspend(std::coroutine_handle<PromiseType> h) noexcept {
            ...
            std::cout << "FinalAwaiter call" << std::endl;
...
public:
    ...
    struct AwaiterBase : public detail::LazyAwaiterBase<T> {
        ...
        AS_INLINE auto await_suspend(std::coroutine_handle<PromiseType>
            ...
            // coroutine_handle 句柄链式传递
            // 协程A 中co_await 协程B 通过 协程B的await_suspend
            // 协程B的promise保存上一次caller(即协程A的coroutine_handle)
    ...
    private:
        ...
        auto awaitSuspendImpl() noexcept(!reschedule) {
            // 核心时if constexpr 条件编译 返回值类型不同 C++17可以处理
            if constexpr (reschedule) {
                ...
                pr._executor->schedule(
                    this->_handle);  // operator () -->call resume()
            } else {
                return this->_handle;
            }
        }
    ...
    void start(F&& callback)
        requires(std::is_invocable_v<F &&, Try<T>>)
    {
        ...
        auto launchCoro = [](LazyBase lazy,
            ...
            // 等价于
            // auto ret = co_await lazy.coAwaitTry()；
            // cb (ret);
        };
        // 通过Lambda  mock 一个立即执行的launchCoro 协程
...
/* czh self read:
 * use common func call a detached coroutine by lazy<T>.start()
 * this detached coroutine function use co_await to get Awaiter value then cb is
 * called param (the Awaiter value)
 *
 * */

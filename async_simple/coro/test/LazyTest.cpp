...
public:
    ...
    void applyValue(std::function<void(int x)> f) {
        std::thread([this, f = std::move(f)]() {
            ...
            spdlog::info("call before applyValue thread_id:{}",
                         get_current_thread_id());
            _cv.wait(l, [this]() { return _done; });
            f(_value);
            spdlog::info("call applyValue thread_id:{}",
                         get_current_thread_id());
        }).detach();
    }
    ...
    Lazy<int> getValue() {
        ...
        auto ret = co_await ValueAwaiter(this);
        spdlog::info("getValue thread_id:{}", get_current_thread_id());
        co_return ret;
    }
    ...
    Lazy<int> plusOne() {
        ...
        co_return v + 1;  // 通过 co_return 设置promise 的value
    }
...
TEST_F(LazyTest, testSimpleAsync) {
    auto test = [this]() -> Lazy<int> {
        ...
        spdlog::info("testSimpleAsync before plusOne thread_id:{}",
                     get_current_thread_id());
        auto ret = co_await plusOne();
        spdlog::info("testSimpleAsync after plusOne ret:{} thread_id:{}", ret,
                     get_current_thread_id());
        CHECK_EXECUTOR(&_executor);
        co_return ret;
    };
    spdlog::info("testSimpleAsync call triggerValue thread_id:{}",
                 get_current_thread_id());

    triggerValue(100);
    ASSERT_EQ(101, syncAwait(test().via(&_executor)));  // 异步跨线程协程
...
TEST_F(LazyTest, testVia) {
    auto test = [this]() -> Lazy<int> {
        ...
        EXPECT_TRUE(tid == std::this_thread::get_id());  // 都是
        co_return ret;
    };
    triggerValue(100);
...
TEST_F(LazyTest, testYield) {
    ...
    auto test1 = [this](std::mutex& m, int& value) -> Lazy<void> {
        m.lock();
        // push task to queue's tail
        co_await Yield();
        value++;
        spdlog::info("this thread:{} value:{}", get_current_thread_id(), value);
        co_return;
    };
    ...
    test1(m1, value1).via(&executor).start([this](Try<void> result) {
        spdlog::info("value1 thread:{}", get_current_thread_id());
    });

    spdlog::info("main thread:{}", get_current_thread_id());
